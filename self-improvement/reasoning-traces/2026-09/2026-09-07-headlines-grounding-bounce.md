...
2. **A pull receipt named the wrong tool.** The draft attributed three live comment quotes to `search_facebook_ad_comments_semantic`. The call did run and the quotes were real, but the Parker hook log does not record semantic calls, so the gate could not see it and read the receipt as false. Candidate rule: when a comment quote has to carry a live receipt, run `search_facebook_ad_comments_sql` (substring) as the citable pull; use semantic to find, SQL to cite.
